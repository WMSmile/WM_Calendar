...
- (NSDate *)date
{
    ...
    c.month = self.month;
    c.day = self.day;
    return [[CalendarManger sharedManager].calendar dateFromComponents:c];
}
...
- (NSString *)toString
{
    ...
    NSString *string = [NSCalendar stringFromDate:date];
    return string;
}
...
- (NSString *)getWeek
{
    ...
    NSString *week_str = [calendar compareIfTodayByDate:date];
    return week_str;
}

//判断是不是同一天
- (BOOL)isEqualTo:(WM_CalendarDayModel *)day
{
...
@implementation WM_CalendarDayModel (Calendar)
//计算当前日期之前几天或者是之后的几天（负数是之前几天，正数是之后的几天）
+ (NSMutableArray *)reloadCalendarView:(NSDate *)date  selectDate:(NSDate *)selectdate needDays:(int)days_number byCalendar:(NSCalendar *)calendar
{
    //如果为空就从当天的日期开始
    if(date == nil){
        date = [NSDate date];
    ...
    //默认选择中的时间
    if (selectdate == nil) {
        selectdate = [NSDate dateWithTimeIntervalSince1970:0];
    }
    
    NSDate *today = date;//起始日期
    NSDate *before = [calendar dayInTheFollowingDay:days_number Date:today];//计算它days天以后的时间
    NSDate *select = selectdate;//选择的日期
    
    NSDateComponents *todayDC= [calendar YMDComponentsByDate:today];
    
    NSDateComponents *beforeDC= [calendar YMDComponentsByDate:before];
    ...
    for (int i = 0; i <= months; i++) {
        ...
        NSDate *month = [calendar dayInTheFollowingMonth:i Date:today] ;
        NSMutableArray *calendarDays = [[NSMutableArray alloc]init];
        
        [self calculateDaysInPreviousMonthWithDate:month andArray:calendarDays byCalendar:calendar];
        [self calculateDaysInCurrentMonthWithDate:month andArray:calendarDays today:today before:before select:select byCalendar:calendar];
        [self calculateDaysInFollowingMonthWithDate:month andArray:calendarDays byCalendar:calendar];//计算下月份的天数
...
+ (void)calculateDaysInPreviousMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array byCalendar:(NSCalendar *)calendar
{
    NSUInteger weeklyOrdinality = [calendar weeklyOrdinalityByDate:[calendar firstDayOfCurrentMonthByDate:date] ];//计算这个的第一天是礼拜几,并转为int型
    NSDate *dayInThePreviousMonth = [calendar dayInThePreviousMonthByDate:date];//上一个月的NSDate对象
    NSUInteger daysCount = [calendar numberOfDaysInCurrentMonthByDate:dayInThePreviousMonth];//计算上个月有多少天
    NSUInteger partialDaysCount = weeklyOrdinality - 1;//获取上月在这个月的日历上显示的天数
    NSDateComponents *components = [calendar YMDComponentsByDate:dayInThePreviousMonth];//获取年月日对象
    
    for (long i = daysCount - partialDaysCount + 1; i < daysCount + 1; ++i) {
        
        WM_CalendarDayModel *calendarDay = [WM_CalendarDayModel calendarDayWithYear:components.year month:components.month day:i];
        calendarDay.style = WM_CellDayTypeEmpty;//不显示
...
+ (void)calculateDaysInFollowingMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array byCalendar:(NSCalendar *)calendar
{
    NSUInteger weeklyOrdinality =  [calendar weeklyOrdinalityByDate: [calendar lastDayOfCurrentMonthByDate:date]];
    if (weeklyOrdinality == 7) return ;
    
    NSUInteger partialDaysCount = 7 - weeklyOrdinality;
    NSDateComponents *components = [calendar YMDComponentsByDate: [calendar dayInTheFollowingMonthByDate:date]];
    
    for (int i = 1; i < partialDaysCount + 1; ++i) {
        WM_CalendarDayModel *calendarDay = [WM_CalendarDayModel calendarDayWithYear:components.year month:components.month day:i];
        calendarDay.style = WM_CellDayTypeEmpty;
        [array addObject:calendarDay];
    }
...
//计算当月的天数
+ (void)calculateDaysInCurrentMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array  today:(NSDate*)today before:(NSDate*)before select:(NSDate*)select byCalendar:(NSCalendar *)calendar
{
    
    NSUInteger daysCount = [calendar numberOfDaysInCurrentMonthByDate:date];//计算这个月有多少天
    NSDateComponents *components = [calendar YMDComponentsByDate:date];//今天日期的年月日
    ...
    for (int i = 1; i < daysCount + 1; ++i) {
        ...
        calendarDay.week = [calendar getWeekIntValueWithDateByDate: [calendarDay date]];
        [self LunarForSolarYear:calendarDay];
        [self changStyle:calendarDay today:today before:before select:select byCalendar:calendar];
        [array addObject:calendarDay];
    }
}
...
+ (void)changStyle:(WM_CalendarDayModel *)calendarDay today:(NSDate*)today before:(NSDate*)before select:(NSDate*)select byCalendar:(NSCalendar *)calendar
{
    
    NSDateComponents *calendarToDay  = [calendar YMDComponentsByDate:today];//今天
    NSDateComponents *calendarbefore = [calendar YMDComponentsByDate:before];//最后一天
    NSDateComponents *calendarSelect = [calendar YMDComponentsByDate:select];//默认选择的那一天
    ...
    if(calendarSelect.year == calendarDay.year &
       ...
       calendarSelect.day == calendarDay.day){
        ...
        if (calendarToDay.year >= calendarDay.year &
            ...
            calendarDay.style = WM_CellDayTypePast;
            ...
            if (calendarDay.week == 1 || calendarDay.week == 7){
                calendarDay.style = WM_CellDayTypeWeek;
                
                //工作日
            }else{
                calendarDay.style = WM_CellDayTypeFutur;
            }
        }
    }
...
+(void)LunarForSolarYear:(WM_CalendarDayModel *)calendarDay{
